...
import { getAllConfigs, adminWithdraw } from "../../services/systems.service";
...
export default function AdminWithdraw() {
  ...
  const minus = (id) => {
    const updateData = receiveData.map((item) => {
      if (item.ID === id) {
        if (item.receive_amount - 1 <= item.maximum_amount)
          return {
            ...item,
  ...
  const deposit = async () => {
    ...
    const res = await adminWithdraw(data);
    if (!res._error) {
      setPaymentStatus(1);
      onSuccess();
      setTimeout(() => {
            ...
            <div key={v.ID} style={{ marginBottom: 10 }}>
              <Input.Group compact style={{ display: "flex" }}>
                ...
                <Button
                  ...
                  type="primary"
                  style={{ marginLeft: 10 }}
                  disabled={v.current_amount <= 0 || paymentStatus !== 0}
                >
                  +
                </Button>
              </Input.Group>
